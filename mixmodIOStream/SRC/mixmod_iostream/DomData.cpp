...
DomData::DomData(string &sFilename) : xmlpp::Document() { set_internal_subset(sFilename, "", ""); }
...
DataDescription *DomData::readDataFile(const string &sFilename)
{
	...
	return new DataDescription(iNbSample, iNbColumn, vColumnDescription, iFormat, iDataFilename, iName);
}
	...
	for (int64_t i = 0; i < nbColumn; ++i) {
		...
		case (IOStreamColumnType::Individual): {
			...
			for (int64_t j = 0; j < ICD->getIndividualDescription().size(); ++j) {
				...
				// index of individual
				individualName->set_attribute("Num", std::to_string(ICD->getIndividualDescription()[j].num));
