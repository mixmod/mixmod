...
DomLabel::DomLabel(string &sFilename) : xmlpp::Document() { set_internal_subset(sFilename, "", ""); }
...
DomLabel::DomLabel(xmlpp::Element *root) { _root = create_root_node_by_import(root); }
...
unique_ptr<LabelDescription> DomLabel::readLabelAsData(string sFilename)
{
	...
	if (elementListColumn) {
		...
		for (auto it = children.begin(); it != children.end(); ++it) {
			...
			if (!n)
				continue;
			// Column Type
			IOStreamColumnType columnType = StringToColumnType(n->get_attribute_value("type", "xsi"));
			...
			case IOStreamColumnType::Qualitative: {
				...
				// creation of Column
				QualitativeColumnDescription *column = new QualitativeColumnDescription(index, nbFactor);
	...
	unique_ptr<LabelDescription> newLabelDescription(
	    new LabelDescription(nbSample, iNbColumn, vColumnDescription, format, dataFilename, sName));
...
unique_ptr<LabelDescription> DomLabel::readLabel(string sFilename)
{
	...
	if (_root->get_name() != "Label" /*&&_root->get_name() != "Partition"*/)
		throw IOStreamErrorType::badElementInDataXML;
	...
	unique_ptr<LabelDescription> newLabelDescription(
	    new LabelDescription(nbSample, 1, vColumnDescription, format, dataFilename, sName));
	return newLabelDescription;
...
void DomLabel::writeListColumnNode(const vector<ColumnDescription *> &vColumnDescription)
{
	...
	for (int64_t i = 0; i < nbColumn; ++i) {
		...
		case (IOStreamColumnType::Qualitative): {
			...
			// nbFactor
			QualitativeColumnDescription *QCD = dynamic_cast<QualitativeColumnDescription *>(vColumnDescription[i]);
			...
			for (std::size_t j = 0; j < QCD->getVariableDescription().size(); ++j) {
				...
				if (!QCD->getVariableDescription()[j].name.empty()) {
					if (!listFactorName)
						listFactorName = column->add_child("ListFactorName");
					xmlpp::Element *factorName = listFactorName->add_child("FactorName");
					// index of factor
					factorName->set_attribute("Num", std::to_string(QCD->getVariableDescription()[j].num));
