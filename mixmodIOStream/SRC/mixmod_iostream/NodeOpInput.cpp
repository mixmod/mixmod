...
NodeOpInput::NodeOpInput(ClusteringInput *input, string &s) : NodeInput(input, s)
{
...
NodeOpInput::NodeOpInput(LearnInput *input, string &s) : NodeInput(input, s)
{
	writeListModel(input);
	writePartitionNode(input, s);
	writeCriterionNode(input);
...
NodeOpInput::NodeOpInput(PredictInput *input, string &s) : NodeInput(input, s) { writeParameterNode(input, s); }
...
PredictInput *NodeOpInput::readXmlPredictInput()
{
	...
	if (dataDescription.getDataType() == HeterogeneousData) {
		...
		for (int i=0; i<cData->getPbDimension(); i++){
		  const XEM::QualitativeColumnDescription *ccd = dynamic_cast<const
		XEM::QualitativeColumnDescription*>(dataDescription.getColumnDescription(i)); Global::vNbFactor.push_back(ccd==nullptr
		? 0 : ccd->getNbFactor());
		  }*/
	}
...
void NodeOpInput::writeListModel(Input *input)
{
	...
	// EDDAModel
	if (!EDDAModel->get_first_child())
		listModel->remove_child(EDDAModel);
	...
	// binaryModel
	if (!binaryModel->get_first_child())
		listModel->remove_child(binaryModel);
	// binaryModel
	if (!compositeModel->get_first_child())
		listModel->remove_child(compositeModel);
	if (!listModel->get_first_child())
		_rootInput->remove_child(listModel);
}
...
void NodeOpInput::writeInitNode(xmlpp::Element *strategyElement, ClusteringInput *cInput, string &sFilename)
{
	...
	case USER_PARTITION: {
		...
		for (int64_t i = 0; i < strategy->getStrategyInit()->getNbPartition(); ++i) {
			...
			string tag = strategy->getStrategyInit()->getPartition(i)->getPartitionFile()._type == TypePartition::label
			                 ? "Label"
			                 : "Partition";
			string extn = strategy->getStrategyInit()->getPartition(i)->getPartitionFile()._type == TypePartition::label
			                  ? ".mxl"
			                  : ".mxd";
...
void NodeOpInput::readModelNode(Input &input)
{
	...
	for (auto it = children.begin(); it != children.end(); ++it) {
		...
		if (!elementTypeModel)
			continue; // the current node isn't an element
		...
		for (auto it2 = m_children.begin(); it2 != m_children.end(); ++it2) {
			...
			if (!elementModel)
				continue;
			if (elementModel->get_name() != "Model")
				continue;
...
void NodeOpInput::readInitNode(ClusteringStrategy *strat, xmlpp::Element *n)
{
	...
	if (n->get_name() != "Init")
		return;
	// initName
	strat->setStrategyInitName(StringToStrategyInitName(n->get_attribute_value("type", "xsi")));
	...
	case SMALL_EM: {
		...
		if (stopRuleNode) {
			...
			if (iteration && epsilon) { // NBITERATION_EPSILON
				...
				strat->setNbIterationInInit(
				    std::stoll(iteration->get_child_text()->get_content()));                 // iteration.text().toLongLong());
...
void NodeOpInput::readAlgoNode(ClusteringStrategy *strat, xmlpp::Element *n)
{
	...
	for (auto it = children.begin(); it != children.end(); ++it) {
		...
		case CEM: {
			...
			// change AlgoName
			strat->setAlgo(StringToAlgoName(algoNode->get_attribute_value("type", "xsi")), compt);
...
void NodeOpInput::readCriterionNode(Input &input)
{
	...
	for (auto it = children.begin(); it != children.end(); ++it) {
		...
		if (n->get_name() == "Criterion" &&
		    find(vCriterionNameTmp.begin(), vCriterionNameTmp.end(), sCriterionName) == vCriterionNameTmp.end()) {
			if (compt == 0) {
...
void NodeOpInput::readPartitionNodeImpl(NumericPartitionInfo &partitionInfo, xmlpp::Element *elementPartition,
                                        TypePartition::TypePartition ty)
{
