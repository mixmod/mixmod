...
template <class T> T *NodeOpOutput::read4Output(Input *inp)
{
	...
	if (elementModel) {
		...
		if (!pInput)
			throw IOStreamErrorType::badLoadXML; // TO DO: find a better error type...
	...
	if (elementError) {
		...
		// Criterion
		xmlpp::Element *elementListCriterion =
		    dynamic_cast<xmlpp::Element *>(_rootOutput->get_first_child("ListOutputCriterion"));
		if (elementListCriterion) {
			...
			for (auto it = children.begin(); it != children.end(); ++it) {
				...
				if (!elementOutputCriterion)
					continue;

				// criterion Name
				xmlpp::Element *cnElt =
				    dynamic_cast<xmlpp::Element *>(elementOutputCriterion->get_first_child("CriterionName"));
				...
				xmlpp::Element *elementError =
				    dynamic_cast<xmlpp::Element *>(elementOutputCriterion->get_first_child("Error"));
				if (elementError) {
					// error
					delete error;
					...
					// criterion Value
					xmlpp::Element *cvElt =
					    dynamic_cast<xmlpp::Element *>(elementOutputCriterion->get_first_child("CriterionValue"));
		...
		return new T(modelType, nbCluster, vCriterionOutput, likelihood,
		             parameterFilename == "" ? *parDesc : *readParameter(parameterFilename), *readLabel(labelFilename),
		             *readProba(probaFilename));
	}
}
...
template <class T> static std::string join(T vect, int64_t sz, std::string sep)
{
...
template <class T>
void NodeOpOutput::writeOutput(T *output, const std::vector<CriterionName> &criterionName, string str, int64_t numOutput)
{
	// strategy
	...
	if (output->getStrategyRunError() == NOERROR) {
		...
		DomParameter docParameter(output->getParameterDescription(), parameterFilename); //.toStdString());
...
template void NodeOpOutput::writeOutput<ClusteringModelOutput>(ClusteringModelOutput *output,
                                                               const std::vector<CriterionName> &criterionName, string str,
                                                               int64_t numOutput);
...
// writePredictOutput
void NodeOpOutput::writePredictOutput(PredictModelOutput *output, string str, int64_t numOutput)
{
