...
enum class IOStreamColumnType { Qualitative, Quantitative, Individual, Weight, Unused };

enum class ProjectType { Clustering, Learn, Predict, Unknown };
...
enum class IOStreamXMLFile { Project, Data, Partition, Parameter, Weights, Label };
...
ClusteringMain *IStream(const std::string &s, // OBSOLETE!!
                        IOStreamFormat format = defaultIOStreamFormat, bool bOnlyInput = false,
...
void OStream(const string &s, IOStreamFormat format = defaultIOStreamFormat, ClusteringMain *cMain = NULL,
             IoMode iomode = IoMode::NUMERIC);
