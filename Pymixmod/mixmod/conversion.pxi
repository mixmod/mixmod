...
cdef mxm.BinaryData *data2binary(data):
    ...
    tmp_mx = [] #np.empty([nb_sample, nb_var], dtype=np.int)
    ...
    #for k in data.columns:
        ...
        tmp_vec = np.empty(nb_sample, dtype=np.int)
        tmp_vec = data.ix[:,k].cat.codes
        tmp_mx.append(tmp_vec)
        #tmp_mx[:,k] = data.ix[:,k].cat.codes
    matrix =  <int64_t**>malloc(nb_sample*sizeof(int64_t*))
    for i in range(nb_sample):
        matrix[i] = <int64_t*>malloc(nb_var*sizeof(int64_t))
        for j in range(nb_var):
            #matrix[i][j] = data.ix[i,j]
            matrix[i][j] = 1  + tmp_mx[j][i] #data.ix[:,j].cat.codes[i]
...
cdef mxm.CompositeData *data2composite(data):
    ...
    for i in range(nb_sample):
        ...
        for j in range(nb_col):
            ...
            if factor[j]:
                b_matrix[i][b_j] = 1 + tmp_mx[j][i] #data.ix[:,j].cat.codes[i]
                nb_modality.push_back(factor[j])
                b_j += 1
            else:
                g_matrix[i][g_j] = data.ix[i,j]
                g_j += 1
