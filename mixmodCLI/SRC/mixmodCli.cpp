...
void showTestsHelp() {
  ...
  cout << "-b, --binaryIn : when set, float values in the input XML file are interpreted as 'hex binary' (in order to avoid precision loss). When not set, the format is 'human readable'"<<endl;
  cout << "-B, --binaryOut : when set,  output float values are generated as 'hex binary' (in order to avoid precision loss). When not set, it follows -b"<<endl;
