...
// Constructor
//------------
LearnMain::LearnMain() { THROW(OtherException, internalMixmodError); }

//------------
// Constructor
//------------
LearnMain::LearnMain(LearnInput *input, LearnOutput *output) : _input(input), _output(output) {}
...
void LearnMain::run(int seed, IoMode iomode, int verbose, int massiccc)
{
	...
	// get labels
	Partition *inputKnownPartition = new Partition(_input->getKnownLabelDescription()->getLabel(), vNbCluster[0]);
	...
	if (_input->getDataType() == QualitativeData && DATA_REDUCE) {
		...
		try {
			...
			workingData = bData->reduceData(correspondenceOriginDataToReduceData, inputKnownPartition, inputInitPartition,
		...
		for (int iModelType = 0; iModelType < nbModelType; iModelType++) {
			...
			estimations[iModelType] = new BinaryModel(modelType, nbCluster, workingData, workingKnownPartition,
			                                          correspondenceOriginDataToReduceData);
		}
	}
	// create model for quantitative data
		...
		for (int iModelType = 0; iModelType < nbModelType; iModelType++) {
			...
			estimations[iModelType] = new Model(modelType, nbCluster, inputData, inputKnownPartition);
		}
	}
		...
		if ((estimations[iModel]->getErrorType()) == NOERROR) {
			...
			for (unsigned int iCriterion = 0; iCriterion < criterion.size(); iCriterion++) {
				...
				if (MASSICCC == 1) {
					...
					progressFile << "{ \"Progress\" :  "
					             << ((double)iCriterion + 1 + (double)iModel * (double)criterion.size()) /
					                    ((double)criterion.size() * (double)nbEstimation) * 100.0;
					time_t currTime;
					time(&currTime);
					double timePerModel =
					    difftime(currTime, startTime) / ((double)iCriterion + 1 + (double)iModel * (double)criterion.size());
					progressFile << ", \"Estimated remaining time\" : "
					             << timePerModel * (criterion.size() - iCriterion - 1) +
					                    (nbEstimation - iModel - 1) * criterion.size() * timePerModel
					             << " } ";
					progressFile.close();
				}
			...
			for (unsigned int iCriterion = 0; iCriterion < criterion.size(); iCriterion++) {
				_output->getLearnModelOutput(iModel)->setCriterionOutput(
				    CriterionOutput(criterion[iCriterion], 0.0, estimations[iModel]->getErrorType()));
			} // end iCriterion
		}
