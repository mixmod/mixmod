...
public:
	...
	static std::map<InputError, const char *> create_map()
	{
		...
		m.insert(
		    std::make_pair(endDataFileReach,
		                   "the end of data file has been reached before reading all samples] verify nbSample or data file"));
		...
		m.insert(std::make_pair(tooManySampleInInitPartitionAndTooManyClusterNotRepresented,
		                        "Error : error in USER_PARTITION initialization : Too many sample in InitPartition and too "
		                        "many cluster not repented"));
		...
		m.insert(std::make_pair(errorInPartitionInput,
		                        "Error in partition file : there is not enough lines in the file (nbSample is required)"));
		m.insert(std::make_pair(badNumberOfValuesInLabelInput, "Error in label file : the number of values bust be nbSample"));
		m.insert(
		    std::make_pair(notEnoughValuesInProbaInput,
		                   "Error in proba file : there is not enough values in the file (nbSample*nbCluster is required)"));
		...
		m.insert(std::make_pair(
		    ...
		    "Error : bad stop type with SEM : this algortihm must be stopped after a predefined number of iterations"));
		m.insert(std::make_pair(badAlgorithmInHDContext, "Error : bad algorithm in HD context : only M or MAP is available"));
		m.insert(std::make_pair(differentSubDimensionsWithMAP,
		                        "Error : given subDimensions in init file and input file are different"));
		m.insert(std::make_pair(wrongSubDimension, "Error : Wrong sub dimension type for given model"));
		m.insert(std::make_pair(missingRequiredInputs,
		                        "Error : Missing required inputs (data, nbSample, pbDimension, tabNbCluster, nbNbCluster)"));
		...
		m.insert(
		    std::make_pair(HDModelsAreNotAvailableInClusteringContext, "HD Models Are Not Available In Clustering Context"));
