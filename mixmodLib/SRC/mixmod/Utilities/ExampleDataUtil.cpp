...
struct GenericData {
	...
	GenericData(int64_t nbSample, int64_t **bData, double **gData, int64_t bNbColumn, int64_t gNbColumn,
	            vector<int64_t> *modality, vector<int64_t> *labels)
	{
...
DataDescription *getXEMDataDescriptionFromGeneric(GenericData *genericData)
{
	...
	case QualitativeData: {
		BinaryData *data =
		    new BinaryData(genericData->nbSample, genericData->bNbColumn, *(genericData->modality), genericData->bData);
		dataDescription = new DataDescription(data);
		delete data;
		break;
	}
	case QuantitativeData: {
		GaussianData *data = new GaussianData(genericData->nbSample, genericData->gNbColumn, genericData->gData);
		dataDescription = new DataDescription(data);
		delete data;
		break;
	}
	case HeterogeneousData: {
		BinaryData *bData =
		    new BinaryData(genericData->nbSample, genericData->bNbColumn, *(genericData->modality), genericData->bData);
		GaussianData *gData = new GaussianData(genericData->nbSample, genericData->gNbColumn, genericData->gData);
