...
bool ModelType::operator==(const ModelType &modelType) const
{
	...
	if (_nameModel != modelType.getModelName())
		return false;
	if (_nbSubDimensionFree != modelType._nbSubDimensionFree)
		return false;
	if (_subDimensionEqual != modelType._subDimensionEqual)
		return false;
	if (_tabSubDimensionFree) {
		for (int64_t i = 0; i < _nbSubDimensionFree; i++) {
			if (_tabSubDimensionFree[i] != modelType.getTabSubDimensionFreeI(i))
				return false;
		}
	}
	return true;
...
{
	if (!isHD(_nameModel) || !isFreeSubDimension(_nameModel))
		THROW(InputException, wrongModelInSetSubDimensionFree);
...
{
	if (!isHD(_nameModel) || isFreeSubDimension(_nameModel))
		THROW(InputException, wrongModelInSetSubDimensionEqual);
