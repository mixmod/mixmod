...
CompositeParameter::CompositeParameter(const CompositeParameter *param) : Parameter(param->getModel(), param->getModelType())
{
	_parameterComponent.resize(2);
	_parameterModelType.resize(2);
	_parameterComponent[0] = (const_cast<CompositeParameter *>(param)->getBinaryParameter())->clone();
	_parameterComponent[1] = ((const_cast<CompositeParameter *>(param)->getGaussianParameter()))->clone();
...
CompositeParameter::CompositeParameter(const Parameter *igaussian, const Parameter *ibinary, ModelType *imodelType)
    : Parameter(ibinary->getNbCluster(), ibinary->getPbDimension() + igaussian->getPbDimension(), imodelType)
{
	_parameterComponent.resize(2);
	_parameterModelType.resize(2);
	_parameterComponent[0] = (const_cast<Parameter *>(ibinary)->getBinaryParameter())->clone();
	_parameterComponent[1] = ((const_cast<Parameter *>(igaussian)->getGaussianParameter()))->clone();
	_parameterModelType[0] = new ModelType(_parameterComponent[0]->getModelType()->_nameModel);
	_parameterComponent[0]->setModelType(_parameterModelType[0]);
	_parameterModelType[1] = new ModelType(_parameterComponent[1]->getModelType()->_nameModel);
	_parameterComponent[1]->setModelType(_parameterModelType[1]);
...
CompositeParameter::CompositeParameter(Model *iModel, ModelType *iModelType, int64_t *tabNbModality)
...
CompositeParameter::~CompositeParameter()
{
	for (unsigned int i = 0; i < _parameterComponent.size(); ++i) {
		if (_parameterComponent[i])
			delete _parameterComponent[i];
		if (_parameterModelType[i])
			delete _parameterModelType[i];
	}
}
...
double CompositeParameter::getLogLikelihoodOne() const
{
	...
	return _parameterComponent[0]->getLogLikelihoodOne() + _parameterComponent[1]->getLogLikelihoodOne();
}

void CompositeParameter::InstantiateBinaryandGaussianParamter(ModelType *modeltype, int64_t *tabNbModality)
{
	...
	case Heterogeneous_p_E_L_B: {
		...
		_parameterComponent[0] = new BinaryEParameter(_model, _parameterModelType[0], tabNbModality);
	...
	case Heterogeneous_p_Ek_L_B: {
		_parameterModelType[0] = new ModelType(Binary_p_E);
		_parameterModelType[1] = new ModelType(Gaussian_p_L_B);
		_parameterComponent[0] = new BinaryEkParameter(_model, _parameterModelType[0], tabNbModality);
		_parameterComponent[1] = new GaussianDiagParameter(_model, _parameterModelType[1]);
		break;
	}
	...
	case Heterogeneous_p_Ej_L_B: {
		_parameterModelType[0] = new ModelType(Binary_p_E);
		_parameterModelType[1] = new ModelType(Gaussian_p_L_B);
		_parameterComponent[0] = new BinaryEjParameter(_model, _parameterModelType[0], tabNbModality);
		_parameterComponent[1] = new GaussianDiagParameter(_model, _parameterModelType[1]);
		break;
	}
	...
	case Heterogeneous_p_Ekj_L_B: {
		_parameterModelType[0] = new ModelType(Binary_p_E);
		_parameterModelType[1] = new ModelType(Gaussian_p_L_B);
		_parameterComponent[0] = new BinaryEkjParameter(_model, _parameterModelType[0], tabNbModality);
		_parameterComponent[1] = new GaussianDiagParameter(_model, _parameterModelType[1]);
		break;
	}
	...
	case Heterogeneous_p_Ekjh_L_B: {
		_parameterModelType[0] = new ModelType(Binary_p_E);
		_parameterModelType[1] = new ModelType(Gaussian_p_L_B);
		_parameterComponent[0] = new BinaryEkjhParameter(_model, _parameterModelType[0], tabNbModality);
		_parameterComponent[1] = new GaussianDiagParameter(_model, _parameterModelType[1]);
		break;
	}
...
{
	int64_t freeparam =
	    _parameterComponent[1]->getFreeParameter() + _parameterComponent[0]->getFreeParameter() - (_nbCluster - 1);
	return freeparam;
}
...
void CompositeParameter::updateForInitRANDOMorUSER_PARTITION(Sample **tabSampleForInit, bool *tabClusterToInitialize)
{
