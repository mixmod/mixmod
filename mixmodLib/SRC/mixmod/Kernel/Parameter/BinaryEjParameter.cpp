...
// Default constructor
//--------------------
BinaryEjParameter::BinaryEjParameter() { THROW(OtherException, wrongConstructorType); }

//-------------------------------
// Constructor called by XEMModel
//-------------------------------
BinaryEjParameter::BinaryEjParameter(Model *iModel, ModelType *iModelType, int64_t *tabNbModality)
...
// copy Constructor
//-----------------
BinaryEjParameter::BinaryEjParameter(const BinaryEjParameter *iParameter) : BinaryParameter(iParameter)
{
	_scatter = new double[_pbDimension];
...
bool BinaryEjParameter::operator==(const BinaryEjParameter &param) const
{
	if (!BinaryParameter::operator==(param))
		return false;
	for (int64_t j = 0; j < _pbDimension; j++) {
		if (_scatter[j] != param.getScatter()[j])
			return false;
	}
	return true;
}
...
void BinaryEjParameter::editScatter(int64_t k)
{
	...
	for (j = 0; j < _pbDimension; j++) {
		for (h = 1; h <= _tabNbModality[j]; h++) {
			if (h == _tabCenter[k][j]) {
				MIXMOD_COUT << "\t" << _scatter[j];
			} else {
				MIXMOD_COUT << "\t" << _scatter[j] / (_tabNbModality[j] - 1);
			}
		}
		MIXMOD_COUT << endl;
	}
...
//---------------------------
void BinaryEjParameter::inputScatter(std::ifstream &fi, int64_t k) { THROW(OtherException, internalMixmodError); }

// Read Scatter in input containers
//---------------------------
void BinaryEjParameter::inputScatter(double ***scatters) { THROW(OtherException, internalMixmodError); }
