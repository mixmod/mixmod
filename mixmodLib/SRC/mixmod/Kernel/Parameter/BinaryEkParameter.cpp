...
BinaryEkParameter::BinaryEkParameter() { THROW(OtherException, wrongConstructorType); }

//-------------------------------
// Constructor called by XEMModel
//-------------------------------
BinaryEkParameter::BinaryEkParameter(Model *iModel, ModelType *iModelType, int64_t *tabNbModality)
...
// copy Constructor
//-----------------
BinaryEkParameter::BinaryEkParameter(const BinaryEkParameter *iParameter) : BinaryParameter(iParameter)
{
	_scatter = new double[_nbCluster];
...
bool BinaryEkParameter::operator==(const BinaryEkParameter &param) const
{
	if (!BinaryParameter::operator==(param))
		return false;
	for (int64_t k = 0; k < _nbCluster; k++) {
		if (_scatter[k] != param.getScatter()[k])
			return false;
	}
	return true;
}
...
//---------------------------
void BinaryEkParameter::inputScatter(std::ifstream &fi, int64_t k) { THROW(OtherException, internalMixmodError); }

// Read Scatter in input containers
//---------------------------
void BinaryEkParameter::inputScatter(double ***scatters) { THROW(OtherException, internalMixmodError); }
