...
//------------
// Default constructor
Parameter::Parameter() { THROW(OtherException, wrongConstructorType); }
...
{
	if (_pbDimension != param.getPbDimension())
		return false;
	if (_nbCluster != param.getNbCluster())
		return false;
	if (_freeProportion != param.getFreeProportion())
		return false;
	for (int64_t k = 0; k < _nbCluster; k++) {
		if (_tabProportion[k] != param.getTabProportion()[k])
			return false;
	}
	return true;
}
...
// generateRandomIndex
//-------------------
int64_t Parameter::generateRandomIndex(bool *tabIndividualCanBeUsedForInitRandom, double *weight, double totalWeight)
{
...
// i : 0 ->_nbSample-1
//-----------------------------------------
void Parameter::computeTikUnderflow(int64_t i, double **tabTik) { THROW(OtherException, nonImplementedMethod); }
