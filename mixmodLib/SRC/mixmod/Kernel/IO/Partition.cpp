...
Partition::Partition(const Label *label, int64_t nbCluster)
{
	if (label != NULL) {
		...
		for (int64_t i = 0; i < _nbSample; i++) {
			...
			if (vLabel[i] < 0 ||
			    vLabel[i] > _nbCluster) { // rlebret 22/11/2013 - after bug Rmixmod semi-supervised --> replace <= by < here
...
// Constructor
//-----------
Partition::Partition(int64_t nbSample, int64_t nbCluster, const NumericPartitionFile &partitionFile)
{
	_nbSample = nbSample;
	_nbCluster = nbCluster;
	_tabValue = NULL;
