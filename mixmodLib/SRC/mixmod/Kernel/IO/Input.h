...
inline int64_t Input::getNbSample() const { return _nbSample; }

inline int64_t Input::getPbDimension() const { return _pbDimension; }

inline std::vector<int64_t> Input::getNbCluster() const { return _nbCluster; }

inline int64_t Input::getNbCluster(int64_t index) const { return _nbCluster[index]; }

inline int64_t Input::getNbClusterSize() const { return _nbCluster.size(); }

inline int64_t Input::getNbCriterion() const { return _criterionName.size(); }

inline std::vector<ModelType *> Input::getModelType() const { return _modelType; }

inline Partition *Input::getKnownPartition() const { return _knownPartition; }

inline const LabelDescription *Input::getKnownLabelDescription() const { return _knownLabelDescription; }
inline LabelDescription *Input::getKnownLabelDescriptionNC() const { return _knownLabelDescription; }

inline DataType Input::getDataType() const { return _dataDescription.getDataType(); }

inline bool Input::isFinalized() const { return _finalized; }

inline const DataDescription &Input::getDataDescription() const { return _dataDescription; }

inline Data *Input::getData() const { return _dataDescription.getData(); }

inline std::vector<CriterionName> const &Input::getCriterionName() const { return _criterionName; }
