...
//  Copy constructor
//-----------------
ModelOutput::ModelOutput(const ModelOutput &modelOutput) { THROW(OtherException, internalMixmodError); }
...
//------------------------------
ModelOutput::ModelOutput(ModelType &modelType, int64_t nbCluster, std::vector<CriterionOutput *> &criterionOutput,
...
ModelOutput::~ModelOutput()
{
	...
	if (_parameterDescription != NULL)
		delete _parameterDescription;
	if (_probaDescription != NULL)
...
bool ModelOutput::operator==(const ModelOutput &modelOutput) const
{
	...
	if (_nbCluster != modelOutput.getNbCluster())
		return false;
	if (!(_modelType == modelOutput.getModelType()))
		return false;
	...
	if (!(_parameterDescription == modelOutput.getParameterDescription()))
		return false;
	if (!(_labelDescription == modelOutput.getLabelDescription()))
		return false;
	if (!(_probaDescription == modelOutput.getProbaDescription()))
		return false;
	return true;
}
