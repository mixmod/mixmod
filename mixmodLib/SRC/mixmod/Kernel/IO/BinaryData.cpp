                             ...
                             Partition *initPartition, Partition *&oKnownPartition, Partition *&oInitPartition)
{
	...
	if (initPartition) {
		...
		for (i = 0; i < _nbSample; i++) {
			for (k = 0; k < nbCluster; k++)
				oInitPartition->_tabValue[correspondenceOriginDataToReduceData[i]][k] = initPartition->_tabValue[i][k];
		}
	...
	if (knownPartition) {
		...
		for (i = 0; i < _nbSample; i++) {
			for (k = 0; k < nbCluster; k++)
				oKnownPartition->_tabValue[correspondenceOriginDataToReduceData[i]][k] = knownPartition->_tabValue[i][k];
		}
