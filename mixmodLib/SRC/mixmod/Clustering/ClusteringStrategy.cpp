...
ClusteringStrategy *ClusteringStrategy::clone() { return new ClusteringStrategy(*this); }
...
void ClusteringStrategy::setTabInitParameter(Parameter **tabInitParameter, int64_t nbInitParameter)
{
...
// set init partition
void ClusteringStrategy::setInitPartition(Partition *part, int64_t position) { _strategyInit->setPartition(part, position); }
...
void ClusteringStrategy::setStrategyInit(StrategyInitName initName, Data *&data, int64_t nbNbCluster, int64_t *tabNbCluster,
                                         ModelType *modelType)
{
	...
	case USER:
		...
		for (int64_t k = 0; k < nbNbCluster; k++) {
			if (isEDDA(modelType->_nameModel)) {
				tabInitParameter[k] = new GaussianGeneralParameter(tabNbCluster[k], pbDimension, modelType, fileName);
			} else if (getModelGenre(modelType->_nameModel) == QualitativeModel) {
				int64_t *tabNbModality = (data->getBinaryData())->getTabNbModality();
				tabInitParameter[k] =
				    new BinaryEkjhParameter(tabNbCluster[k], pbDimension, modelType, tabNbModality, fileName);
			} else if (isHD(modelType->_nameModel)) {
				tabInitParameter[k] = new GaussianHDDAParameter(tabNbCluster[k], pbDimension, modelType, fileName);
			} else
				THROW(OtherException, internalMixmodError);
		}
...
{
	if ((_strategyInit->getStrategyInitName() == USER) || (_strategyInit->getStrategyInitName() == USER_PARTITION)) {
		THROW(InputException, badSetNbTry);
	}
	if (nbTry < minNbTryInStrategy) {
		THROW(InputException, nbTryInStrategyTooSmall);
	} else if (nbTry > maxNbTryInStrategy) {
...
//---------------
void ClusteringStrategy::input_FLAT_FORMAT(std::ifstream &fi, Data *&data, int64_t nbNbCluster, int64_t *tabNbCluster,
                                           ModelType *modelType)
{
	int64_t j;
	std::string keyWord = "";
...
int64_t ClusteringStrategy::getNbTryInInit() const { return _strategyInit->getNbTry(); }

int64_t ClusteringStrategy::getNbIterationInInit() const { return _strategyInit->getNbIteration(); }

double ClusteringStrategy::getEpsilonInInit() const { return _strategyInit->getEpsilon(); }

void ClusteringStrategy::setNbTryInInit(int64_t nbTry) { _strategyInit->setNbTry(nbTry); }

void ClusteringStrategy::setStrategyInitName(StrategyInitName initName) { _strategyInit->setStrategyInitName(initName); }

void ClusteringStrategy::setNbIterationInInit(int64_t nbIteration) { _strategyInit->setNbIteration(nbIteration); }

void ClusteringStrategy::setEpsilonInInit(double epsilon) { _strategyInit->setEpsilon(epsilon); }

AlgoStopName ClusteringStrategy::getStopNameInInit() const { return (_strategyInit->getStopName()); }

void ClusteringStrategy::setStopNameInInit(AlgoStopName stopName) { _strategyInit->setStopName(stopName); }
