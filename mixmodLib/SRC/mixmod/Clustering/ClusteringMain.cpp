...
ClusteringMain::ClusteringMain() { THROW(OtherException, internalMixmodError); }

ClusteringMain::ClusteringMain(ClusteringInput *input, ClusteringOutput *output) : _input(input), _output(output) {}
...
#endif
	{
		...
		if (_input->getDataType() == QualitativeData && DATA_REDUCE) {
			...
			try {
				...
				workingData = bData->reduceData(correspondenceOriginDataToReduceData, workingKnownPartition,
		...
		for (int nbCluster_i = 0; nbCluster_i < nbnbCluster; nbCluster_i++) {
			for (int nbModel_i = 0; nbModel_i < nbModel; nbModel_i++) {
				...
				case QualitativeData:
					model = new BinaryModel(modelType, nbCluster, workingData, workingKnownPartition,
					                        correspondenceOriginDataToReduceData);
					break;
				case QuantitativeData:
					model = new Model(modelType, nbCluster, workingData, workingKnownPartition);
					break;
				...
				if (model->getErrorType() == NOERROR) {
					...
					for (std::vector<CriterionName>::const_iterator it = vCriterion.begin(); it != vCriterion.end(); it++) {
						CriterionName criterion = *it;
						cmoutput->setCriterionOutput(CriterionOutput(criterion, 0.0, model->getErrorType()));
					}
				}
				...
				if (MASSICCC == 1) {
					...
					progressFile << "{ \"Progress\" :  "
					             << ((double)count + 1.0) / ((double)nbModel * (double)nbnbCluster) * 100.0;
