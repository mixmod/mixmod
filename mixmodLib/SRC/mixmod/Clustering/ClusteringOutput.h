...
public:
	...
	// Add one ClusteringModelOutput (incremental construction). Count is needed because vector.puch_back won't work with
	// openMP
...
inline int64_t ClusteringOutput::getNbClusteringModelOutput() const { return _clusteringModelOutput.size(); }

inline int ClusteringOutput::getCriterionSize() const { return _criterionName.size(); }

inline const CriterionName &ClusteringOutput::getCriterionName(const int index) const { return _criterionName[index]; }

inline const std::vector<CriterionName> &ClusteringOutput::getCriterionName() const { return _criterionName; }
