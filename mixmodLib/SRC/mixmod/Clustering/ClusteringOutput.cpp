...
ClusteringOutput::ClusteringOutput(const ClusteringOutput &cOutput)
    : _clusteringModelOutput(cOutput.getClusteringModelOutput()), _criterionName(cOutput.getCriterionName())
{
}

ClusteringOutput *ClusteringOutput::clone() { return new ClusteringOutput(*this); }
...
void ClusteringOutput::clusteringModelOutputResize(int64_t size) { _clusteringModelOutput.resize(size); }

//---------------------------
// Initialization Constructor (unused. TODO: remove)
//---------------------------
ClusteringOutput::ClusteringOutput(std::vector<Model *> const &estimations, std::vector<CriterionName> const &criterionName)
    : _clusteringModelOutput(estimations.size()), _criterionName(criterionName)
{
	...
	for (uint64_t k = 0; k < _clusteringModelOutput.size(); k++) {
		...
		for (int64_t i = 0; i < maxNbCriterion; i++) {
			for (int64_t j = 0; j < maxNbCriterion; j++) {
				if (cOutputThis->getCriterionOutput(i).getCriterionName() ==
					...
					if (fabs(cOutputThis->getCriterionOutput(i).getValue() - cOutputOther->getCriterionOutput(j).getValue()) >
					    precision) {
						cout << "UNEQUAL: criterion differ res" << k + 1 << "  : computed "
		...
		vector<vector<double>> probaThis = cOutputThis->getProbaDescription()->getProba()->getProba();
		vector<vector<double>> probaOther = cOutputOther->getProbaDescription()->getProba()->getProba();
		for (int64_t i = 0; i < nbSample; i++) {
		...
		if (isBinary(cOutputThis->getParameterDescription()->getModelType()->getModelName())) {
			...
			for (int64_t j = 0; j < nbCluster; j++) {
				...
				if (fabs((bParameterThis->getTabProportion())[j] -
					...
					cout << "UNEQUAL: proportions differ resParam" << k + 1 << ", cluster " << (j + 1) << ": computed "
				...
				for (int64_t h = 0; h < pbDimension; h++) {
					if (fabs((bParameterThis->getTabCenter())[j][h] -
						...
						cout << "UNEQUAL: means differ resParam" << k + 1 << ", cluster " << (j + 1) << " Variable " << (h + 1)
				...
				for (int64_t h = 0; h < pbDimension; h++) {
					...
					for (int64_t m = 0; m < tabNbModality; m++) {
						if (fabs(scatterThis[j][h][m] - scatterOther[clustersCorrespondence[j]][h][m]) > precision) {
							cout << "UNEQUAL: Scatter differ resParam" << k + 1 << ", cluster " << (j + 1) << " Scatter["
							     << h + 1 << "][" << m + 1 << "] "
				...
				for (int64_t l = 0; l < pbDimension; l++) {
					for (int64_t m = 0; m < pbDimension; m++) {
						if (fabs(storeThis[l][m] - storeOther[l][m]) > precision) {
							cout << "UNEQUAL: Sigma differ resParam" << k + 1 << ", cluster " << (j + 1) << " Sigma[" << l + 1
							     << "][" << m + 1 << "] "
